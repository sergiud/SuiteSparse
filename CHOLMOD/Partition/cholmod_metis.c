...
// During test coverage in CHOLMOD/Tcov, the call to malloc inside METIS
// pretends to fail, to test CHOLMOD's memory handling.  This causes METIS to
// terminate the program.  To avoid this, METIS is allowed to use the standard
// ANSI C11 malloc/calloc/realloc/free functions during testing.
...
// METIS will terminate your program if it runs out of memory.  In an attempt
// to workaround METIS' behavior, this routine allocates a single block of
// memory of size equal to an observed upper bound on METIS' memory usage.  It
// then immediately deallocates the block.  If the allocation fails, METIS is
// not called.
//
// Median memory usage for a graph with n nodes and nz edges (counting each
// edge twice, or both upper and lower triangular parts of a matrix) is 4*nz +
// 40*n + 4096 integers.  A "typical" upper bound is 10*nz + 50*n + 4096
// integers.  Nearly all matrices tested fit within that upper bound, with the
// exception two in the SuiteSparse matrix collection: Schenk_IBMNA/c-64 and
...
)
{
    ...
    nn = n ;
    TEST_COVERAGE_PAUSE ;
    int metis_result = METIS_ComputeVertexSeparator (&nn,
...
)
{
    ...
    else if (Common->metis_nswitch > 0)
    {
        ...
        // The workaround is to return the identity permutation instead of
        // using METIS for matrices of dimension 3000 or more and with density
        // of 66% or more - admittedly an uncertain fix, but such matrices are
        // so dense that any reasonable ordering will do, even identity (n^2 is
        // only 50% higher than nz in this case).  CHOLMOD's nested dissection
        // method (cholmod_nested_dissection) has no problems with the same
        // matrix, even though it too uses METIS_ComputeVertexSeparator.  The
        // matrix is derived from LPnetlib/lpi_cplex1 in the SuiteSparse matrix
        // collection.  If C is the lpi_cplex matrix (of order 3005-by-5224), A
        // = (C*C')^2 results in the seg fault.  The seg fault also occurs in
        // the stand- alone onmetis program that comes with METIS.  If a future
        // version of METIS fixes this problem, then set Common->metis_nswitch
        // to zero.
    ...
    {
        #ifdef DUMP_GRAPH
        printf ("Calling METIS_NodeND n "ID" nz "ID" density " // DUMP_GRAPH
        ...
        nn = n ;
        TEST_COVERAGE_PAUSE ;
        metis_result = METIS_NodeND (&nn, Mp, Mi,
