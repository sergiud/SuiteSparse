function compare_symbolic(N, cond)
% Usage :
%           compare_symbolic
%           compare_symbolic(N, cond)
...
fprintf('ID \t Size \t \t skysize1 \t skysize2 \t #givens1 \t #givens2 \t #swaps1 \t #swaps2 \t flops1 \t #flops2 \n\n') 
...
for id = ids(1:N)
    ...
    R = qr(A2) ;
    [rR, cR] = size(R)  ;
    [toprow1, ng1, nswap1, fl1] = blksky_symbolic(R) ;

    csize1 = 0 ;
    for k = 1 : cR
        csize1 = csize1 + k - toprow1(k) + 1 ;
    end



    fprintf('%d \t %d-by-%d \t %g \t\t %g \t\t %g \t\t %g \t\t %g \t\t %g \t\t %g \t\t %g \n',...
    Prob.id, m, n, csize, csize1, ng, ng1, nswap, nswap1, fl, fl1  ) 
end
