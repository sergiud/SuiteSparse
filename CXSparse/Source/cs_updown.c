...
CS_INT cs_updown (cs *L, CS_INT sigma, const cs *C, const CS_INT *parent)
{
    ...
    for (j = f ; j != -1 ; j = parent [j])          /* walk path f up to root */
    {
        ...
        gamma = CS_DIV (CS_MUL (CS_MAKE_ENTRY ((double) sigma),
            CS_CONJ (alpha)), CS_MAKE_ENTRY (beta2 * beta)) ;
        Lx [p] = CS_ADD (CS_MUL (CS_MAKE_ENTRY (delta), Lx [p]),
            (sigma > 0) ? CS_MUL (gamma, w [j]) : CS_ZERO ()) ;
        beta = beta2 ;
#ifdef CS_COMPLEX
