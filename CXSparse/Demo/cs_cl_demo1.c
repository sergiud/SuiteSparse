// CXSparse/Demo/cs_cl_demo1: demo program for CXSparse (complex int64_t)
...
int main (void)
{
    ...
    for (i = 0 ; i < m ; i++)
        cs_cl_entry (T, i, i, CS_COMPLEX_ONE ()) ;
    Eye = cs_cl_compress (T) ;             /* Eye = speye (m) */
    cs_cl_spfree (T) ;
    C = cs_cl_multiply (A, AT) ;           /* C = A*A' */
    D = cs_cl_add (C, Eye, CS_COMPLEX_ONE (),
        CS_COMPLEX_MAKE_ENTRY (cs_cl_norm (C))) ;   /* D = C + Eye*norm (C,1) */
